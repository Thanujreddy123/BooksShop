...
function getPrevious(order) {
    ...
    if (!order) { // No previous order so display defaults    
        ...
        previous.book = getBooks();     // Display initial list of books with none selected
    }
    else {  // Redisplay existing values 
        
        previous.quantity = order.quantityString; // Redisplay original quantity in form
        
        var books = getBooks(); // Get copy of books list    
        for (let b of books) {
            if (order.id === b.id) { // Search for books matching id of previous selection
